...
QIcon loadAppIcon(const QString &applicationDirPath) {
    const QStringList candidates = {
        QDir(applicationDirPath).filePath(QStringLiteral("assets/GnssView.png")),
...
int main(int argc, char *argv[]) {
    ...
    QGuiApplication::setDesktopFileName(QStringLiteral("com.hdgnss.GnssView"));
#endif
    ...
    if (!engine.rootObjects().isEmpty()) {
        if (auto *window = qobject_cast<QWindow *>(engine.rootObjects().constFirst())) {
            ...
            const auto applyWindowStyle = [window, &settings]() {
                hdgnss::applyPlatformWindowStyle(window, settings.uiTheme() == QStringLiteral("light"));
            };
            applyWindowStyle();
            QObject::connect(&settings, &hdgnss::AppSettings::uiThemeChanged, window, applyWindowStyle);
        }
    }
    return app.exec();
}
