...
- `.github/workflows/linux-packages.yml` builds DEB and RPM packages, uploads them as artifacts, and uploads them as release assets for `v*` tags. The DEB job uses an Ubuntu 26.04 container so the distro Qt packages satisfy the Qt 6.5 minimum.
...
- macOS and Windows release packages include GnssView, QML, assets, and Qt runtime files. Linux DEB/RPM packages include GnssView, QML, and assets, and use distribution Qt packages. Release packages do not include external plugin binaries by default.
...
- `include/hdgnss`
- `packaging/`
  - `linux/`: DEB/RPM desktop metadata and package description.
  - `macos/`: signing and DMG packaging metadata.
  - `windows/`: NSIS installer source.
- `scripts/`
- `src/core`
- `src/models`
- `src/protocols`
