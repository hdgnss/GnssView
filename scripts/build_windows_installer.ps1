...
$stagingDir = Join-Path $BuildDir "packaging\windows\stage\$Configuration"
$outputDir = Join-Path $BuildDir "packaging\windows\output"
$version = Get-ProjectVersion -CMakeFile (Join-Path $repoRoot "CMakeLists.txt")
$installerScript = Join-Path $repoRoot "packaging\windows\GnssView.nsi"
