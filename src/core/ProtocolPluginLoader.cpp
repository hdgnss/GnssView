...
QStringList ProtocolPluginLoader::defaultSearchPaths(const QString &applicationDirectoryPath,
...
void ProtocolPluginLoader::loadFromDirectory(const QString &directoryPath) {
    loadFromDirectories({directoryPath});
}

void ProtocolPluginLoader::loadFromDirectories(const QStringList &directoryPaths) {
    ...
    for (const QString &rawPath : directoryPaths) {
        ...
        for (const QFileInfo &entry : entries) {
            ...
            if (!hasExpectedPluginIid(*loader, QString::fromLatin1(HDGNSS_PROTOCOL_PLUGIN_IID))) {
                continue;
            }
            ...
            IProtocolPlugin *plugin = qobject_cast<IProtocolPlugin *>(instance);
            if (!plugin) {
                m_errors.append(QStringLiteral("%1: not a HDGNSS protocol plugin").arg(entry.fileName()));
                continue;
            }
...
QList<IProtocolPlugin *> ProtocolPluginLoader::plugins() const {
    return m_plugins;
}

QList<QObject *> ProtocolPluginLoader::pluginObjects() const {
    return m_pluginObjects;
}

QStringList ProtocolPluginLoader::errors() const {
    return m_errors;
}

QStringList ProtocolPluginLoader::loadedDirectories() const {
