...
QString normalizeUiTheme(const QString &theme) {
    return theme.trimmed().toLower() == QStringLiteral("light")
...
void AppSettings::setUiTheme(const QString &theme) {
    const QString normalized = normalizeUiTheme(theme);
    if (m_uiTheme == normalized) {
        return;
    }
    m_uiTheme = normalized;
    storeValue(QStringLiteral("app/uiTheme"), m_uiTheme);
    emit uiThemeChanged();
}
...
void AppSettings::load() {
    ...
    m_uiTheme = normalizeUiTheme(settings.value(QStringLiteral("app/uiTheme"), QStringLiteral("dark")).toString());
