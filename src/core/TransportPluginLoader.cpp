...
QStringList TransportPluginLoader::defaultSearchPaths(const QString &applicationDirectoryPath,
...
void TransportPluginLoader::loadFromDirectory(const QString &directoryPath) {
    loadFromDirectories({directoryPath});
}

void TransportPluginLoader::loadFromDirectories(const QStringList &directoryPaths) {
    ...
    for (const QString &rawPath : directoryPaths) {
        ...
        for (const QFileInfo &entry : entries) {
            ...
            if (!hasExpectedPluginIid(*loader, QString::fromLatin1(HDGNSS_TRANSPORT_PLUGIN_IID))) {
                continue;
            }
            ...
            ITransportPlugin *plugin = qobject_cast<ITransportPlugin *>(instance);
            if (!plugin) {
                m_errors.append(QStringLiteral("%1: not a HDGNSS transport plugin").arg(entry.fileName()));
                continue;
            }
...
QList<ITransportPlugin *> TransportPluginLoader::plugins() const {
    return m_plugins;
}

QList<QObject *> TransportPluginLoader::pluginObjects() const {
    return m_pluginObjects;
}

QStringList TransportPluginLoader::errors() const {
    return m_errors;
}

QStringList TransportPluginLoader::loadedDirectories() const {
